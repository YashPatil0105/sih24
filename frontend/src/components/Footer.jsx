...
export const Footer = () => {
  ...
  <span
    className="mines"
    style={{
      backgroundImage: "url('https://cdn.pixabay.com/photo/2018/08/25/20/58/wall-3630911_1280.jpg')",
    ...
    Mines
          ...
          <div>
            ...
            <ul className="space-y-2">
              {['Home', 'Estimation', 'Carbon Neutrality', 'Simulator', 'Carbon Credits'].map((item, index) => (
                <li key={index}>
                  <Link to={`/${item.toLowerCase().replace(' ', '')}`} className="text-gray-300 hover:text-white transition-colors duration-300">
                    {item}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" className="bg-green-500 hover:bg-green-600 text-white px-6 py-2 rounded-md transition-colors duration-300">
                Subscribe
              </button>
            </form>
          </div>
